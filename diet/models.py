...
class Meal_record(models.Model): # 식사기록 테이블
    ...
    date =              models.DateField(verbose_name='입력날짜') # Python의 datetime.date인스턴스로 '날짜'
    time =              models.DateTimeField(verbose_name='입력시간') # Python의 datetime.datetime 인스턴스로 '날짜'와 '시간'
    photo_file =        models.ImageField(upload_to='photos/%Y/%m/%d', blank=True, null=True)
